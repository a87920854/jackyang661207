 ...
 **************************************************************** **/
	if(jQuery('#preloader').length > 0) {
		...
		jQuery(document).ready(function() {
			...
			jQuery('#preloader').fadeOut(1500, function() {
...
// bxslider
(function ($) {
	var _container = jQuery(".consult-slide-main");
	if(_container.length > 0) {		
		loadScript(plugin_path + 'jquery.bxslider.min.js', function() {
			$('.consult-slide-main').bxSlider({
				auto: true,
				pause: 8000,
				mode: 'fade',
				pager: false,
...
(function ($) {
	if($("#register").length > 0){
		...
		switch(href){
			case "consult.html" || "consult.asp":
				$("#class1").attr("checked","checked");
				$(".class1-metoo").text("體驗諮詢")
				$("#class1").click(function(e){e.preventDefault()});
				break;
			case "class.html" || "class.asp":
				$("#class2").attr("checked","checked");
				$(".class2-metoo").text("體驗課程")
				$("#class2").click(function(e){e.preventDefault()});
				break;
			case "date.html" || "date.asp":			
				$("#class3").attr("checked","checked");
				$(".class3-metoo").text("體驗排約")
				$("#class3").click(function(e){e.preventDefault()});
				break;
			default:
				return;
		}
...
(function ($) {
	if($("#genderSelect").length > 0){
		...
		$("#genderSelect").on("change", function(){
			...
			switch(choose){
				case "0":
					$consult.text(price[0].consult);
					$class.text(price[0].class);
					$dating.text(price[0].dating);
					$word.hide();
					break;
				case "1":
					$consult.text(price[1].consult);
					$class.text(price[1].class);
					$dating.text(price[1].dating);
					$word.show();
					break;
				default:
					$consult.text(price[2].consult);
